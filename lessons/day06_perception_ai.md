...
机器学习会从例子中调整规则。今天只要能说清输入、输出、评价方式，以及结果怎样交给控制器。模仿学习、强化学习和具体模型名称，放在[扩展资料](../EXTENSIONS.md)。

## 第6步：需要时再看数据集

第一遍只看清数据里的任务和字段。论文、外部数据集和训练细节见[扩展资料](../EXTENSIONS.md)。
...
需要网络训练、数据集或策略部署细节时，去[扩展资料](../EXTENSIONS.md)。
