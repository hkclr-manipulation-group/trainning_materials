...
现在有17本可编辑的 Notebook：入门操作1本、七天实验7本、深入专题5本、连续带练4本。文字、公式、图像和程序放在同一页；按 Shift+Enter 运行代码块，修改参数后重新运行就会得到新的图和数值。新增13–16保存默认输出，便于先读结果，再亲手修改。

## 直接开始连续带练

先读[11门正文中的33节带练](ENGINEERING_PATH.md)，再在对应Notebook复算、修改和回放故障。每本新Notebook有9个代码单元（含环境准备），默认输出中的曲线来自实际离线运行。
