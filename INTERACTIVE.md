...
用浏览器阅读讲义、运行 Python 和修改例子。基础课只需要[00号课堂](notebooks/00_start_here.ipynb)和七本当天实验；其他 Notebook 属于扩展。

按 Shift+Enter 运行代码。先运行默认示例，再改一个参数，预测变化后重新运行。

## 先做基础课

按[七课路线](WEEK_PLAN.md)进入当天 Notebook。每本课堂都可以单独运行，不连接真实设备。

## 后续专题

完成基础课后，再读[11门工程课程](ENGINEERING_PATH.md)和对应的连续带练。下面四本课堂用于通信、EtherCAT、电机控制和系统联调；它们需要更多专业背景。

| 课堂 | 从什么做到什么 | 学员修改任务 |
|---|---|---|
| [13 CAN FD](notebooks/13_canfd_workshop.ipynb) | 手算补码→12字节解码→节点路由→回绕/重复→接收年龄→预算图 | 加节点、错ID、截断帧、改频率 |
| [14 EtherCAT](notebooks/14_ethercat_workshop.ipynb) | PDO偏移→符号解析→状态掩码→联合门禁→WKC→周期图 | 改映射、丢失模式条件、改变计算耗时 |
| [15 电机控制](notebooks/15_control_workshop.ipynb) | P环递推→不可达目标→PI积分饱和→恢复指标→停止距离 | 改惯量/限值、比较积分、解释速度平方项 |
| [16 系统联调](notebooks/16_system_workshop.ipynb) | 标定拟合→梯度更新→TCP→时间插值→路径检查→执行条件 | 退化数据、时间缺口、薄障碍、旧场景 |

四本课都不连接设备。默认输出便于阅读；修改代码或参数后，原输出会过时，应重新运行并保存自己的副本。
...
|---|---|---|---|
| 本机JupyterLab | 启动服务的电脑 | 本机Python包、文件、命令行工具 | 已提供启动器与17本教材 |
